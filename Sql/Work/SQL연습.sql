1. emp 테이블에서 직원의 이름(ename), 급여(sal)와 입사일(hiredate)만을 출력하는 select  문을 작성하시오. 
...
select * from emp where ename = '송혜교';
select * from emp;
select * from emp where ename like '__헌%';
...
select deptno, avg(sal), sum(sal), max(sal), min(sal), count(*) from emp
...
insert into emp
      (empno, ename)
values((select max(empno)+1 from emp e),'아이린');
...
select * from emp where deptno = (select deptno from dept where loc = '인천');
